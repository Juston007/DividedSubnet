...
namespace SubnetConsoleDemo
{
    class Subnet
    {
        ...
        public Subnet(String ipStr,int prefix,int availableHost,String firstHost,String lastHost,String broadcast)
        {
            NetworkID = ipStr;
            this.prefix = prefix;
        ...
        public static Subnet[] DivideSubnet(String ipStr, int dividedPrefix, out String msg)
        {
            //为了方便，首先将IP地址字符串转换为二进制形式
            byte[] ipBytes = Subnet.getByteByDottedDecimal(ipStr);
            ...
            //计算划分后有几个二进制位
            int defaultPrefix = Subnet.getDefaultNetworkPrefix(ipStr);
            if (defaultPrefix <= 0)
            {
            ...
            Subnet[] subnets = new Subnet[(int)Math.Pow(2, borrowedCount)];
            ...
            for (int i = 0; i < subnets.Length; i++)
            {
                ...
                */
                ...
                subnets[i] = new Subnet(ipStr, dividedPrefix,availableHost, subnetFirstHost, subnetLastHost, subnetBroadcast);
            }
